...
bool PlayLayer::init()
{
    ...
    // Yes, you can load this value from other module.　寿司表示個数
    m_width = MATRIX_WIDTH;
    m_height = MATRIX_HEIGHT;
    
    // init position value 寿司の原点
...
//     std vector用
    for (int i=0; i<m_width * m_height; ++i) {
        sushiLinq.push_back(&m_matrix[i]);
    }
//    Cocos     Vector用
//    for (int i=0; i<m_width * m_height; ++i) {
//        sushiLinq.pushBack( Value( &m_matrix[i] ) );
//    }
...
//    SushiSprite *sushi = NULL;
    ...
    auto result = cpplinq::from(sushiLinq)
...
bool PlayLayer::onTouchBegan(Touch *touch, Event *unused)
{
    ...
    m_destSushi = NULL;
    if (m_isTouchEnable) {
        auto location = touch->getLocation();   // 位置取得
        m_srcSushi = sushiOfPoint(&location);   // その位置の寿司取得
    }
...
void PlayLayer::onTouchMoved(Touch *touch, Event *unused)
{
    ...
    int row = m_srcSushi->getRow();     // 寿司の行
    int col = m_srcSushi->getCol();     // 寿司の列
    // タッチ中の位置
    ...
    // not a useful movement　動かさなかった場合、そのままreturn
}
// 寿司の入れ替え
void PlayLayer::swapSushi()
{
    ...
    Point posOfSrc = m_srcSushi->getPosition();     // 寿司の位置
    Point posOfDest = m_destSushi->getPosition();   // 入れ替え先の寿司の位置
    float time = 0.2;
    
    // 1.swap in matrix　中身を入れ替える
    ...
    // 2.check for remove able　消せるかチェック
...
{
    // if animationing, init with false
    if (m_isAnimationing) {
...
{
    chainList.push_back(sushi);// １番目の要素
    // １番目の左隣から
    ...
    while (neighborCol >= 0) {
        ...
        if (neighborSushi
            && (neighborSushi->getImgIndex() == sushi->getImgIndex())   // 同じ寿司
            && !neighborSushi->getIsNeedRemove()                        // 消えてない
            && !neighborSushi->getIgnoreCheck()) {                      // 選択されてない
            chainList.push_back(neighborSushi);                     // リストに追加
            neighborCol--;
        } else {
            break;  // 違う種類の寿司だったら終了
        }
...
void PlayLayer::fillVacancies()
{
    ...
    for (int col = 0; col < m_width; col++) {
        ...
        for (int row = 0; row < m_height; row++) {
            ...
            if (NULL == sushi) {        // 寿司が消えてた
                removedSushiOfCol++;    // 補充カウンタをプラス
            } else {
                if (removedSushiOfCol > 0) {
                    // evey item have its own drop distance　寿司を消えてる個数分ずらす
                    int newRow = row - removedSushiOfCol;
                    // switch in matrix　寿司を代入、元のは消す
    ...
    // 2. create new item and drop down.　新しい寿司を落とす
    for (int col = 0; col < m_width; col++) {
